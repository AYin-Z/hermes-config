...
def _format_ts(ts: int | float | str | None) -> str:
    """Convert a timestamp (Unix ms or ISO datetime string) to a readable local-time string."""
    if not ts:
        return ""
    try:
        if isinstance(ts, str):
            # ISO datetime string from MemOS REST API
...
class MemTensorProvider(MemoryProvider):
    ...
    def __init__(self) -> None:
        ...
        self._bridge_init_failed_permanently = False
        self._session_id = ""
    ...
    def _get_bridge(self):
        """Return the bridge instance, creating it lazily if needed.

        Unlike initialize() which tries once and gives up, this method
        retries on every call — so if MemOS comes online after gateway
        startup, the bridge auto-recovers without a restart.
        """
        if self._bridge is not None:
            return self._bridge

        # Don't hammer a dead endpoint — one retry per call is enough
        if self._bridge_init_failed_permanently:
            return None
        ...
        try:
            ...
            logger.info("MemTensor bridge connected (lazy init)")
            self._bridge_init_failed_permanently = False
            return self._bridge
        except Exception as e:
            logger.warning("MemTensor bridge connection failed (lazy): %s", e)
            self._bridge = None
            return None

    def _clear_bridge_on_error(self, error: Exception) -> None:
        """Clear bridge on connection errors so next call retries."""
        err_str = str(error).lower()
        if any(kw in err_str for kw in ("connection", "refused", "reset", "timeout")):
            self._bridge = None
            logger.info("MemTensor bridge cleared — will retry on next call")
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        # Skip the Node.js daemon when using HTTP transport (REST API).
        # The daemon uses local SQLite; HTTP transport goes to Docker memos-api.
        # Keep the daemon code intact — re-enable by unsetting MEMOS_API_URL.
        if not os.environ.get("MEMOS_API_URL"):
            ...
            logger.info("MEMOS_API_URL set — skipping daemon, using HTTP transport")
        ...
        try:
            ...
            self._bridge = None
            # Don't mark as permanently failed — _get_bridge() will retry
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        ...
        if not bridge:
            return ""
        try:
            result = self._do_recall(query, bridge)
        except Exception as e:
    ...
    def queue_prefetch(self, query: str, *, session_id: str = "") -> None:
        ...
        def _run():
            ...
            try:
                ...
                user_content, assistant_content, sid, turn_id = pending
                messages = []
                if user_content:
                ...
                if messages:
                    bridge.ingest(messages, session_id=sid, turn_id=turn_id, owner=OWNER)
                    bridge.flush()
            except Exception as e:
                self._clear_bridge_on_error(e)
    ...
    def _do_recall(self, query: str, bridge=None) -> str:
        if bridge is None:
            bridge = self._get_bridge()
        if not bridge:
            return ""
        ...
        try:
            search_resp = bridge.search(query, max_results=5, min_score=0.4, owner=OWNER)
        ...
        if not parts:
            try:
                recent_resp = bridge.recent(limit=10, owner=OWNER)
                ...
                for m in memories:
                    ...
                    ts = m.get("createdAt")
                    parts.append(_format_memory_entry(text, role=role, ts=ts))
                recent_fallback = len(parts)
            except Exception as e:
                self._clear_bridge_on_error(e)
                logger.debug("MemTensor recent in prefetch failed: %s", e)

        logger.info(
            "[memtensor] recall | query=%r | search_hits=%d | recent_fallback=%d | injected=%d",
            query[:80], search_hits, recent_fallback, len(parts),
        )
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        A turn_counter ensures each turn has a unique idempotency key
        (session_id + turn_id) to prevent duplicate writes on retry.
        """
        if not self._get_bridge():
            return
        ...
        turn_id = f"{self._turn_counter}"
        self._pending_ingest = (user_content, assistant_content, sid, turn_id)
    ...
    def handle_tool_call(self, tool_name: str, args: dict[str, Any], **kwargs) -> str:
        ...
        bridge = self._get_bridge()
        if not bridge:
            return tool_error("MemTensor bridge not connected — MemOS may be down. Try again later.")

        try:
            from config import OWNER

            resp = bridge.search(query, max_results=8, owner=OWNER)
    ...
    def on_session_end(self, messages: list[dict[str, Any]]) -> None:
        ...
        if pending:
            try:
                ...
                if msgs:
                    bridge.ingest(msgs, session_id=sid, owner=OWNER)
            except Exception as e:
                self._clear_bridge_on_error(e)
